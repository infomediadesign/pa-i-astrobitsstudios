...
void AttackRing::StartBurst(Vector2 bossPos, float innerStart, float outerStart, float innerEnd, float outerEnd, float duration, bool instant) {
    center = bossPos;
    teleActive = false;
    burstActive = true;
...
float AttackRing::CheckDamage(float dt, Vector2 bossPos, Rectangle playerRect) {
    ...
    float minD2 = 1e12f; float maxD2 = 0.0f;
    // approximate by testing rect corners
    Vector2 corners[4] = { {playerRect.x, playerRect.y}, {playerRect.x+playerRect.width, playerRect.y}, {playerRect.x, playerRect.y+playerRect.height}, {playerRect.x+playerRect.width, playerRect.y+playerRect.height} };
    for (int i=0;i<4;i++){
