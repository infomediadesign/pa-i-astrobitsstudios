...
void AttackSwing::Init() { active = false; swingAngle = 0.0f; swingBallPos = {0,0}; timer = 0.0f; }
...
float AttackSwing::CheckDamage(float dt, Vector2 bossPos, Rectangle playerRect) {
    ...
    if (CheckCollisionCircleRec(swingBallPos, swingBallRadius, playerRect)) { active = false; return swingDamage; }
    return 0.0f;
}
