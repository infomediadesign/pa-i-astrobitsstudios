...
void BossAngriff::SetBossHP(float hp, float maxHp) { bossHP = hp; bossMaxHP = (maxHp <= 0.001f) ? 1.0f : maxHp; }
...
void BossAngriff::StartRing1Burst(Vector2 bossPos) {
    StopAllAttacks(*this);
    ringAttack.StartBurst(bossPos, ring1InnerStart, ring1OuterStart, ring1InnerEnd, ring1OuterEnd, ring1BurstDuration, false);
}
...
void BossAngriff::Update(float dt, Vector2 bossPos, Vector2 playerPos, Rectangle playerRect, Player &player, Enemy &boss) {
    ...
    switch(mode)
    {
        ...
        default: mode = MODE_RING1_TELE; break;
    }
}
...
float BossAngriff::CheckDamage(float dt, Vector2 bossPos, Rectangle playerRect) {
    ...
    if (ringAttack.CheckDamage(dt, bossPos, playerRect) > 0) { dmgTimer = 0.5f; return ring1Damage * mult; }
    if (swingAttack.CheckDamage(dt, bossPos, playerRect) > 0) { dmgTimer = 0.5f; return swingDamage * mult; }
